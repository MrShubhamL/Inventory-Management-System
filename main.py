            ...
            if username == row[0] and password == row[1]:
                ...
                for row in data2:
                    if MAC == row[0]:
                        ...
                        sdfds = QMessageBox.information(self,"Unathorized","You have not authoriesed User, Kindlly Contact the Customer Service")
                        sys.exit(Login)
...
class MainApp(QMainWindow,Tk):
    def __init__(self):
        ...
        self.initUI()
        self.setWindowIcon(QtGui.QIcon('icon.ico'))
        self.setWindowTitle("Sivanya Clothing Center")
        # self.showFullScreen()
        self.showMaximized()
        wScr, hScr = autopy.screen.size()

        self.setGeometry(0, 0, wScr, hScr)
        self.setCentralWidget(self.mdi)
        self.Sales()
        self.statusBar().showMessage('You are Successfully LogIn')
        self.show()
...
# ============== Main Menu Bar =======================
    def mainMenu(self):
        ...
        delete_user = QAction("Delete User Account", self)
        # clear_items_table.triggered.connect(self.clear_all_items)
        Acc_bar.addAction(delete_user)
